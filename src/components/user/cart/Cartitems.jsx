...
const CartItems = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      ...
      try {
        ...
        const cartResponse = await fetch(`http://localhost:5000/cart/get-cart`, {
          method: 'POST',
          headers: {
        ...
        const productPromises = uniqueProductIds.map(async (productId) => {
          const productResponse = await fetch('http://localhost:5000/:productId', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
  ...
  const handleVoucherRedeem = async () => {
    try {
      const response = await fetch('http://localhost:5000/coupon/verify-coupon', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
